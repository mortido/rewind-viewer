...
#define PRIMITIVE_ADD_METHOD(METHOD_NAME)                                                 \
  ...
  void METHOD_NAME(Args&&... args) {                                                      \
    if (proto_creation_) {                                                                \
      ...
      frame->primitives_indices_[layer_id_].METHOD_NAME(frame->primitives_storage_,       \
                                                        static_cast<uint32_t>(origin_),   \
...
class SceneEditor {
 private:
  ...
  CameraOrigin origin_ = CameraOrigin::game;
 ...
 public:
  ...
  void set_layer(size_t l, bool p, CameraOrigin origin);
