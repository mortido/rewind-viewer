...
#define PRIMITIVE_ADD_METHOD(METHOD_NAME)                                                 \
  ...
  void METHOD_NAME(Args&&... args) {                                                      \
    if (proto_creation_) {                                                                \
      ...
      frame->primitives_indices_[layer_id_].METHOD_NAME(                                  \
          frame->primitives_storage_, projection_idx_, std::forward<Args>(args)...);       \
    }                                                                                     \
  }
...
class FrameEditor {
 ...
 public:
  ...
  void end_proto() { proto_creation_ = false; }
  void add_proto(size_t idx, glm::vec2 position, float angle, uint32_t color, float scale) {
    ...
    frame->primitives_indices_[layer_id_].copy_from(frame->primitives_storage_, proto_storage_,
                                                    protos_indices_[idx], projection_idx_, position,
                                                    angle, color, scale);
  }
