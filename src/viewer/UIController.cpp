...
void UIController::next_frame(Scene *scene,
                              rewind_viewer::net::NetListener::ConStatus client_status) {
  // Start new frame
...
void UIController::main_menu_bar() {
  if (ImGui::BeginMainMenuBar()) {
    if (ImGui::BeginMenu(ICON_FA_EYE " View", true)) {
      ImGui::Checkbox("Status overlay", &wnd_->show_fps_overlay);
...
void UIController::fps_overlay_widget(rewind_viewer::net::NetListener::ConStatus net_status) {
  ...
  if (wnd_->show_fps_overlay) {
    ...
    if (immediate_mode_enabled()) {
      ImGui::TextColored(mode_color, ICON_FA_EXCLAMATION_CIRCLE " Immediate send mode enabled");
    }
    ImGui::End();
  }
}
...
void UIController::info_widget(Scene *scene) {
  ...
  if (ImGui::CollapsingHeader(ICON_FA_MAP_O " Map", flags)) {
    ImGui::Checkbox("World origin on top left", &conf_->camera.origin_on_top_left);

    ImGui::SetColorEditOptions(ImGuiColorEditFlags_NoInputs);
    ImGui::Checkbox("##show_grid", &conf_->scene.show_grid);
    ImGui::SameLine();
    ImGui::ColorEdit3("Grid", glm::value_ptr(conf_->scene.grid_color));

    ImGui::Checkbox("##draw_map_background", &conf_->scene.draw_background);
    ImGui::SameLine();
  ...
  if (ImGui::CollapsingHeader(ICON_FA_LIST " Layers visibility", flags)) {
    ...
    static const std::array<const char *, static_cast<size_t>(Frame::LAYERS_COUNT)> tick_captions =
    ...
    idx = 0;
    for (bool &enabled : conf_->scene.enabled_permanent_layers) {
