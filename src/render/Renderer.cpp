...
namespace {
using rewind_viewer::render::RenderContext;
...
Renderer::Renderer(const std::string &shaders_dir, glm::u32vec2 area_size, glm::u16vec2 grid_cells)
    :resources_{std::make_unique<ResourceManager>()}
          ,shaders_{std::make_unique<ShadersCollection>(shaders_dir)}
    , ctx_render_params_(RenderContext::create_gl_context(*resources_))
  ...
  attr_->rect_vao = resources_->gen_vertex_array();
  GLuint vbo = resources_->gen_buffer();
  //@formatter:off
  const float points[] = {
  ...
  attr_->uniform_buf = resources_->gen_buffer();
...
void Renderer::update_frustum(const models::Camera &cam) {
...
void Renderer::render_background(glm::vec3 color) {
  std::lock_guard<Spinlock> lock(grid_mutex_);
  // Main scene area
  shaders_->color.use();
...
void Renderer::render_grid(glm::vec3 color) {
  ...
  if (attr_->grid_vao == 0) {
    attr_->grid_vao = resources_->gen_vertex_array();
    GLuint vbo = resources_->gen_buffer();
...
void Renderer::set_canvas_config(const glm::vec2 &size, const glm::u16vec2 &grid) {
  std::lock_guard<Spinlock> lock(grid_mutex_);
  area_size_ = size;
  grid_cells_ = grid;
  attr_->grid_model = glm::scale(glm::mat4{1.0}, {area_size_.x, area_size_.y, 0.0f});
