...
class TriangleT(object):
    ...
    def InitFromBuf(cls, buf, pos):
        triangle = Triangle()
        triangle.Init(buf, pos)
        return cls.InitFromObj(triangle)
    ...
    @classmethod
    def InitFromObj(cls, triangle):
        x = TriangleT()
        x._UnPack(triangle)
        return x

    # TriangleT
    def _UnPack(self, triangle):
        if triangle is None:
            return
        if triangle.Color() is not None:
            self.color = rewind_viewer.fbs.Color.ColorT.InitFromObj(triangle.Color())
        if not triangle.PointsIsNone():
            self.points = []
            for i in range(triangle.PointsLength()):
                if triangle.Points(i) is None:
                    self.points.append(None)
                else:
                    vector2f_ = rewind_viewer.fbs.Vector2f.Vector2fT.InitFromObj(triangle.Points(i))
    ...
    def Pack(self, builder):
        ...
        if self.points is not None:
            TriangleStartPointsVector(builder, len(self.points))
