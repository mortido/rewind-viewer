# automatically generated by the FlatBuffers compiler, do not modify

# namespace: fbs

import flatbuffers
from flatbuffers.compat import import_numpy
np = import_numpy()

class Circle(object):
    __slots__ = ['_tab']

    @classmethod
    def GetRootAs(cls, buf, offset=0):
        n = flatbuffers.encode.Get(flatbuffers.packer.uoffset, buf, offset)
        x = Circle()
        x.Init(buf, n + offset)
        return x

    @classmethod
    def GetRootAsCircle(cls, buf, offset=0):
        """This method is deprecated. Please switch to GetRootAs."""
        return cls.GetRootAs(buf, offset)
    # Circle
    def Init(self, buf, pos):
        self._tab = flatbuffers.table.Table(buf, pos)

    # Circle
    def Color(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(4))
        if o != 0:
            x = self._tab.Indirect(o + self._tab.Pos)
            from rewind_viewer.fbs.Color import Color
            obj = Color()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None

    # Circle
    def Center(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(6))
        if o != 0:
            x = o + self._tab.Pos
            from rewind_viewer.fbs.Vector2f import Vector2f
            obj = Vector2f()
            obj.Init(self._tab.Bytes, x)
            return obj
        return None

    # Circle
    def Radius(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(8))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Float32Flags, o + self._tab.Pos)
        return 0.0

def CircleStart(builder):
    builder.StartObject(3)

def Start(builder):
    CircleStart(builder)

def CircleAddColor(builder, color):
    builder.PrependUOffsetTRelativeSlot(0, flatbuffers.number_types.UOffsetTFlags.py_type(color), 0)

def AddColor(builder, color):
    CircleAddColor(builder, color)

def CircleAddCenter(builder, center):
    builder.PrependStructSlot(1, flatbuffers.number_types.UOffsetTFlags.py_type(center), 0)

def AddCenter(builder, center):
    CircleAddCenter(builder, center)

def CircleAddRadius(builder, radius):
    builder.PrependFloat32Slot(2, radius, 0.0)

def AddRadius(builder, radius):
    CircleAddRadius(builder, radius)

def CircleEnd(builder):
    return builder.EndObject()

def End(builder):
    return CircleEnd(builder)

import rewind_viewer.fbs.Color
import rewind_viewer.fbs.Vector2f
try:
    from typing import Optional
except:
    pass

class CircleT(object):

    # CircleT
    def __init__(self):
        self.color = None  # type: Optional[rewind_viewer.fbs.Color.ColorT]
        self.center = None  # type: Optional[rewind_viewer.fbs.Vector2f.Vector2fT]
        self.radius = 0.0  # type: float

    @classmethod
    def InitFromBuf(cls, buf, pos):
        circle = Circle()
        circle.Init(buf, pos)
        return cls.InitFromObj(circle)

    @classmethod
    def InitFromPackedBuf(cls, buf, pos=0):
        n = flatbuffers.encode.Get(flatbuffers.packer.uoffset, buf, pos)
        return cls.InitFromBuf(buf, pos+n)

    @classmethod
    def InitFromObj(cls, circle):
        x = CircleT()
        x._UnPack(circle)
        return x

    # CircleT
    def _UnPack(self, circle):
        if circle is None:
            return
        if circle.Color() is not None:
            self.color = rewind_viewer.fbs.Color.ColorT.InitFromObj(circle.Color())
        if circle.Center() is not None:
            self.center = rewind_viewer.fbs.Vector2f.Vector2fT.InitFromObj(circle.Center())
        self.radius = circle.Radius()

    # CircleT
    def Pack(self, builder):
        if self.color is not None:
            color = self.color.Pack(builder)
        CircleStart(builder)
        if self.color is not None:
            CircleAddColor(builder, color)
        if self.center is not None:
            center = self.center.Pack(builder)
            CircleAddCenter(builder, center)
        CircleAddRadius(builder, self.radius)
        circle = CircleEnd(builder)
        return circle
